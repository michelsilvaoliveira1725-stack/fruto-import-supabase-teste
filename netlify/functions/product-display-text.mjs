...
export default async (req) => {
  ...
  const r = await fetch(`${SUPABASE_URL}/functions/v1/fruto-product-display-text`, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({ code: body?.code, displayText: body?.displayText }),
    cache: "no-store"
  });
  let data = {};
  try { data = await r.json(); } catch {}
  if (!r.ok) return json({ error: data?.error || "Não foi possível salvar o texto exibido." }, r.status);
  return json(data, 200);
};

export const config = { path: "/api/product-display-text", method: ["POST"] };
